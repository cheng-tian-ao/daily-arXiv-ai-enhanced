...
TL;DR: 本文提出了ABBSPO框架，通过自适应边界框缩放和对称先验角度预测，解决了弱监督定向目标检测中尺度估计不准确和学习崩溃的问题，在HBox监督的OOD任务中取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的水平边界框监督的定向目标检测方法存在两个主要问题：1) 直接将真实水平边界框与预测定向框的最小外接矩形进行比较，导致尺度估计不准确；2) 当所有增强视图的预测都错误时，学习过程会崩溃。

Method: 提出ABBSPO框架，包含两个核心组件：1) 自适应边界框缩放，通过适当缩放真实水平边界框来优化每个预测定向框的尺寸；2) 对称先验角度损失，利用航空目标的固有对称性进行自监督学习，防止学习崩溃。

Result: 广泛的实验结果表明，ABBSPO在弱监督定向目标检测任务中取得了最先进的性能，超越了现有方法。

Conclusion: ABBSPO框架通过自适应缩放和对称先验角度预测，有效解决了弱监督定向目标检测中的关键挑战，为利用现有水平边界框标注进行定向检测提供了高效且准确的解决方案。
...
TL;DR: 本文提出了一种新的图像字幕生成范式SST，将扩散模型作为建议模块辅助自回归生成，而非替代它，在COCO数据集上取得了125.1 CIDEr-D的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 扩散去噪模型在连续域表现出色，但在离散域（如文本生成）中难以超越自回归模型。作者希望结合扩散模型的双向和细化能力与自回归模型的强语言结构优势。

Method: 提出Show, Suggest and Tell (SST)框架：使用扩散模型为自回归生成提供建议，而不是替代它。具体来说，扩散模型作为建议模块，为自回归解码器提供候选词建议。

Result: 在COCO数据集上达到125.1 CIDEr-D（无强化学习），比自回归和扩散模型SOTA分别高出1.5和2.5分。实验显示建议质量与字幕质量呈正相关。

Conclusion: 将扩散模型作为建议模块辅助自回归生成是一个有前景的研究方向，在图像字幕生成任务中展现出优于单一方法的性能。
...
TL;DR: MetaVoxel是一个基于联合扩散模型的生成框架，能够同时建模医学影像和临床元数据的联合分布，实现多种任务的统一处理而无需特定任务训练。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法通常针对特定预测方向训练条件分布模型，需要为不同任务分别训练模型。为了统一医疗AI模型并提高临床适用性，需要开发能够建模联合分布、支持灵活零样本推理的框架。

Method: MetaVoxel是一个生成式联合扩散建模框架，通过单一扩散过程学习覆盖所有变量（包括影像数据和临床元数据）的联合分布。该方法支持使用任意输入子集进行零样本推理，无需特定任务重新训练。

Result: 在超过10,000个T1加权MRI扫描和临床元数据（来自九个数据集）上，单个MetaVoxel模型能够执行图像生成、年龄估计和性别预测，性能与已建立的特定任务基线相当。实验还展示了其在灵活推理方面的能力。

Conclusion: 联合多模态扩散为统一医疗AI模型和实现更广泛临床适用性提供了有前景的方向。MetaVoxel框架能够统一传统上需要单独条件模型的任务，并支持灵活的零样本推理。
...
TL;DR: 提出独立密度估计（IDE）方法来解决大规模视觉语言模型在组合泛化方面的不足，通过建立句子中单个单词与图像特征之间的联系来实现组合泛化


<details>
  <summary>Details</summary>
Motivation: 尽管大规模视觉语言模型在图像描述和条件图像生成等领域取得了显著成果，但在实现类似人类的组合泛化方面仍然存在困难

Method: 提出独立密度估计（IDE）方法，构建两种模型：一种使用完全解耦的视觉表示作为输入，另一种利用变分自编码器从原始图像中获取部分解耦特征；同时提出基于熵的组合推理方法来结合句子中每个单词的预测

Result: 所提模型在多个数据集上表现出优于当前模型的组合泛化能力，能够更好地泛化到未见过的组合

Conclusion: IDE方法能够有效提升视觉语言模型的组合泛化能力，为解决当前模型在组合泛化方面的挑战提供了新的解决方案
...
TL;DR: TraceFlow是一个用于动态高光场景高保真渲染的新框架，通过残差材料增强的2D高斯泼溅表示和混合渲染管线，实现了精确的反射方向估计和物理准确的反射建模。


<details>
  <summary>Details</summary>
Motivation: 当前动态高光场景渲染面临两个关键挑战：精确的反射方向估计和物理准确的反射建模。现有方法在处理复杂动态环境中的镜面反射时，难以同时保证高质量的几何重建和物理真实的反射效果。

Method: 1. 提出残差材料增强的2D高斯泼溅表示，建模动态几何和材料属性以实现精确反射光线计算
2. 引入动态环境高斯和混合渲染管线，将渲染分解为漫反射和镜面反射分量
3. 通过光栅化和光线追踪实现物理基础的镜面合成
4. 设计从粗到精的训练策略以提高优化稳定性并促进物理有意义的分解

Result: 在动态场景基准测试中，TraceFlow在定量和定性评估上都优于现有方法，能够在复杂动态环境中产生更清晰、更真实的镜面反射效果。

Conclusion: TraceFlow通过创新的表示方法和混合渲染架构，有效解决了动态高光场景渲染中的关键挑战，实现了高质量的物理准确反射合成，为动态场景渲染提供了新的解决方案。
...
### [6] [What matters for Representation Alignment: Global Information or Spatial Structure?](https://arxiv.org/abs/2512.10794)
*Jaskirat Singh,Xingjian Leng,Zongze Wu,Liang Zheng,Richard Zhang,Eli Shechtman,Saining Xie*

Main category: cs.CV

TL;DR: 该研究发现，在生成模型训练中，目标表征的空间结构（而非全局语义性能）是影响生成质量的关键因素，并提出了简单改进方法iREPA来增强空间信息传递。


<details>
  <summary>Details</summary>
Motivation: 研究生成训练中表征对齐（REPA）机制的本质：探究目标表征的全局语义信息（如图像分类准确率）与空间结构（补丁标记间的余弦相似度）哪个对生成性能更为重要。传统观点认为更强的全局语义性能会带来更好的生成结果。

Method: 1. 对27种不同视觉编码器进行大规模实证分析，比较全局语义性能与空间结构对生成性能的影响；2. 提出iREPA方法：将REPA中的标准MLP投影层替换为简单卷积层，并引入空间归一化层来增强空间信息传递。

Result: 研究发现与普遍认知相反：空间结构而非全局语义性能是驱动目标表征生成性能的关键因素。iREPA方法（仅需<4行代码修改）在多种视觉编码器、模型规模和训练变体（如REPA、REPA-E、Meanflow、JiT等）中均能一致提高收敛速度。

Conclusion: 该研究挑战了关于表征对齐工作机制的传统认知，强调了空间结构在生成模型训练中的重要性。iREPA的简单改进证明了通过增强空间信息传递可以显著提升生成模型的训练效率，为未来改进生成模型训练方法提供了新思路。
...
TL;DR: 本文提出了分层实例跟踪新任务，创建了首个相关基准数据集，评估了多种模型变体并发现该任务具有挑战性


<details>
  <summary>Details</summary>
Motivation: 现有跟踪任务主要关注物体级别的跟踪，缺乏对物体及其部件之间层次关系的跟踪。为了推动更细粒度和结构化视觉理解的发展，需要建立能够同时跟踪对象及其部件并保持层次关系的新任务和基准

Method: 提出分层实例跟踪新任务，创建包含2,765个独特实体、552个视频、40个类别（涵盖物体和部件）的首个基准数据集，并对四种模型的七个变体进行系统评估

Result: 新数据集具有挑战性，现有模型在该任务上表现有限，需要进一步改进。数据集已公开可用

Conclusion: 分层实例跟踪是一个重要且有挑战性的新任务，提出的基准数据集将为该领域研究提供基础，推动更细粒度和结构化的视觉理解发展
...
TL;DR: Latent-CoT-Drive (LCDrive)提出使用潜在语言而非自然语言进行推理，通过在动作对齐的潜在空间中统一推理和决策，实现更高效的自动驾驶推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在自动驾驶中使用自然语言进行链式思维推理，但文本可能不是最高效的推理表示形式。需要探索更有效的推理表示方法来提升驾驶性能和安全性。

Method: LCDrive在动作对齐的潜在空间中表达链式思维推理，使用两种令牌：(1)动作提议令牌，与模型输出动作共享词汇；(2)世界模型令牌，基于学习的潜在世界模型表达动作的未来结果。通过监督真实未来场景展开来冷启动潜在CoT，然后通过闭环强化学习进行后训练。

Result: 在大规模端到端驾驶基准测试中，LCDrive相比非推理和文本推理基线，实现了更快的推理速度、更好的轨迹质量，并从交互式强化学习中获得了更大的性能提升。

Conclusion: 使用潜在语言而非自然语言进行链式思维推理，通过在动作对齐的潜在空间中统一推理和决策，能够更有效地提升自动驾驶模型的性能和安全性。
...
### [35] [Breaking the Vicious Cycle: Coherent 3D Gaussian Splatting from Sparse and Motion-Blurred Views](https://arxiv.org/abs/2512.10369)
...
### [36] [RaLiFlow: Scene Flow Estimation with 4D Radar and LiDAR Point Clouds](https://arxiv.org/abs/2512.10376)
...
### [37] [Self-Supervised Contrastive Embedding Adaptation for Endoscopic Image Matching](https://arxiv.org/abs/2512.10379)
...
### [38] [Towards Fine-Grained Recognition with Large Visual Language Models: Benchmark and Optimization Strategies](https://arxiv.org/abs/2512.10384)
...
### [39] [Adaptive Dual-Weighted Gravitational Point Cloud Denoising Method](https://arxiv.org/abs/2512.10386)
...
### [40] [MultiHateLoc: Towards Temporal Localisation of Multimodal Hate Content in Online Videos](https://arxiv.org/abs/2512.10408)
...
### [41] [Beyond Endpoints: Path-Centric Reasoning for Vectorized Off-Road Network Extraction](https://arxiv.org/abs/2512.10416)
...
### [42] [TransLocNet: Cross-Modal Attention for Aerial-Ground Vehicle Localization with Contrastive Learning](https://arxiv.org/abs/2512.10419)
...
### [43] [Neural Collapse in Test-Time Adaptation](https://arxiv.org/abs/2512.10421)
...
### [44] [An M-Health Algorithmic Approach to Identify and Assess Physiotherapy Exercises in Real Time](https://arxiv.org/abs/2512.10437)
...
### [45] [Error-Propagation-Free Learned Video Compression With Dual-Domain Progressive Temporal Alignment](https://arxiv.org/abs/2512.10450)
...
### [46] [Robust Shape from Focus via Multiscale Directional Dilated Laplacian and Recurrent Network](https://arxiv.org/abs/2512.10498)
...
### [47] [3D Blood Pulsation Maps](https://arxiv.org/abs/2512.10517)
...
### [48] [Take a Peek: Efficient Encoder Adaptation for Few-Shot Semantic Segmentation via LoRA](https://arxiv.org/abs/2512.10521)
...
### [49] [Blink: Dynamic Visual Token Resolution for Enhanced Multimodal Understanding](https://arxiv.org/abs/2512.10548)
...
### [50] [Grounding Everything in Tokens for Multimodal Large Language Models](https://arxiv.org/abs/2512.10554)
...
### [51] [Data-Efficient American Sign Language Recognition via Few-Shot Prototypical Networks](https://arxiv.org/abs/2512.10562)
*Meher Md Saad*
...
### [52] [Audio-sync Video Instance Editing with Granularity-Aware Mask Refiner](https://arxiv.org/abs/2512.10571)
...
### [53] [Unleashing Degradation-Carrying Features in Symmetric U-Net: Simpler and Stronger Baselines for All-in-One Image Restoration](https://arxiv.org/abs/2512.10581)
...
### [54] [Salient Object Detection in Complex Weather Conditions via Noise Indicators](https://arxiv.org/abs/2512.10592)
...
### [55] [Beyond Pixels: A Training-Free, Text-to-Text Framework for Remote Sensing Image Retrieval](https://arxiv.org/abs/2512.10596)
...
### [56] [Track and Caption Any Motion: Query-Free Motion Discovery and Description in Videos](https://arxiv.org/abs/2512.10607)
...
### [57] [Robust Multi-Disease Retinal Classification via Xception-Based Transfer Learning and W-Net Vessel Segmentation](https://arxiv.org/abs/2512.10608)
...
### [58] [Lang2Motion: Bridging Language and Motion through Joint Embedding Spaces](https://arxiv.org/abs/2512.10617)
...
### [59] [DOCR-Inspector: Fine-Grained and Automated Evaluation of Document Parsing with VLM](https://arxiv.org/abs/2512.10619)
...
### [60] [K-Track: Kalman-Enhanced Tracking for Accelerating Deep Point Trackers on Edge Devices](https://arxiv.org/abs/2512.10628)
...
### [61] [TriDF: Evaluating Perception, Detection, and Hallucination for Interpretable DeepFake Detection](https://arxiv.org/abs/2512.10652)
...
### [62] [NaviHydra: Controllable Navigation-guided End-to-end Autonomous Driving with Hydra-distillation](https://arxiv.org/abs/2512.10660)
...
### [63] [XDen-1K: A Density Field Dataset of Real-World Objects](https://arxiv.org/abs/2512.10668)
...
### [64] [Geo6DPose: Fast Zero-Shot 6D Object Pose Estimation via Geometry-Filtered Feature Matching](https://arxiv.org/abs/2512.10674)
...
### [65] [Sharp Monocular View Synthesis in Less Than a Second](https://arxiv.org/abs/2512.10685)
...
### [66] [CheXmask-U: Quantifying uncertainty in landmark-based anatomical segmentation for X-ray images](https://arxiv.org/abs/2512.10715)
...
### [67] [SpaceDrive: Infusing Spatial Awareness into VLM-based Autonomous Driving](https://arxiv.org/abs/2512.10719)
...
### [68] [Video Depth Propagation](https://arxiv.org/abs/2512.10725)
...
### [69] [IRG-MotionLLM: Interleaving Motion Generation, Assessment and Refinement for Text-to-Motion Generation](https://arxiv.org/abs/2512.10730)
...
### [70] [LDP: Parameter-Efficient Fine-Tuning of Multimodal LLM for Medical Report Generation](https://arxiv.org/abs/2512.10750)
...
### [71] [Blood Pressure Prediction for Coronary Artery Disease Diagnosis using Coronary Computed Tomography Angiography](https://arxiv.org/abs/2512.10765)
...
### [72] [Graph Laplacian Transformer with Progressive Sampling for Prostate Cancer Grading](https://arxiv.org/abs/2512.10808)
...
### [73] [Self-Ensemble Post Learning for Noisy Domain Generalization](https://arxiv.org/abs/2512.10818)
...
### [74] [PoseGAM: Robust Unseen Object Pose Estimation via Geometry-Aware Multi-View Reasoning](https://arxiv.org/abs/2512.10840)
...
### [75] [SWiT-4D: Sliding-Window Transformer for Lossless and Parameter-Free Temporal 4D Generation](https://arxiv.org/abs/2512.10860)
...
### [76] [From Macro to Micro: Benchmarking Microscopic Spatial Intelligence on Molecules via Vision-Language Models](https://arxiv.org/abs/2512.10867)
...
### [77] [MoCapAnything: Unified 3D Motion Capture for Arbitrary Skeletons from Monocular Videos](https://arxiv.org/abs/2512.10881)
...
### [78] [PubTables-v2: A new large-scale dataset for full-page and multi-page table extraction](https://arxiv.org/abs/2512.10888)
...
### [79] [DuetSVG: Unified Multimodal SVG Generation with Internal Visual Guidance](https://arxiv.org/abs/2512.10894)
...
### [80] [FoundationMotion: Auto-Labeling and Reasoning about Spatial Movement in Videos](https://arxiv.org/abs/2512.10927)
...
### [81] [GaussianHeadTalk: Wobble-Free 3D Talking Heads with Audio Driven Gaussian Splatting](https://arxiv.org/abs/2512.10939)
...
### [82] [VL-JEPA: Joint Embedding Predictive Architecture for Vision-language](https://arxiv.org/abs/2512.10942)
...
### [83] [MeViS: A Multi-Modal Dataset for Referring Motion Expression Video Segmentation](https://arxiv.org/abs/2512.10945)
...
### [84] [Towards Efficient and Effective Multi-Camera Encoding for End-to-End Driving](https://arxiv.org/abs/2512.10947)
...
### [85] [ClusIR: Towards Cluster-Guided All-in-One Image Restoration](https://arxiv.org/abs/2512.10948)
...
### [86] [E-RayZer: Self-supervised 3D Reconstruction as Spatial Visual Pre-training](https://arxiv.org/abs/2512.10950)
...
### [87] [Group Diffusion: Enhancing Image Generation by Unlocking Cross-Sample Collaboration](https://arxiv.org/abs/2512.10954)
...
### [88] [Omni-Attribute: Open-vocabulary Attribute Encoder for Visual Concept Personalization](https://arxiv.org/abs/2512.10955)
...
### [89] [Empowering Dynamic Urban Navigation with Stereo and Mid-Level Vision](https://arxiv.org/abs/2512.10956)
...
### [90] [WorldLens: Full-Spectrum Evaluations of Driving World Models in Real World](https://arxiv.org/abs/2512.10958)
...
### [91] [StereoSpace: Depth-Free Synthesis of Stereo Geometry via End-to-End Diffusion in a Canonical Space](https://arxiv.org/abs/2512.10959)
...
### [92] [What Kind of Reasoning (if any) is an LLM actually doing? On the Stochastic Nature and Abductive Appearance of Large Language Models](https://arxiv.org/abs/2512.10080)
*Luciano Floridi,Jessica Morley,Claudio Novelli,David Watson*

Main category: cs.CL

TL;DR: 本文分析当前基于token补全的大语言模型(LLMs)的推理机制，指出它们本质上具有随机性，表面上类似人类溯因推理，但实际上只是基于训练数据中的模式生成文本，而非真正进行推理。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型的推理本质，澄清它们看似具备的溯因推理能力实际上是基于统计模式而非真正的认知过程，这对评估和应用LLMs具有重要意义。

Method: 通过理论分析和实例展示，对比LLMs的token补全机制与人类溯因推理的区别，分析其随机性本质和表面上的推理表现。

Result: LLMs能够生成看似合理的想法、模仿常识推理并提供解释性答案，但这些输出缺乏真实性、语义基础、验证机制和理解能力，并非真正的溯因推理。

Conclusion: LLMs具有随机性基础但表面上表现出溯因推理特征，这种双重性意味着它们可用于辅助人类思考和创意生成，但必须批判性评估其输出，因为它们无法识别真相或验证解释。文章最后回应了五个反对观点，承认分析局限性并给出总体评价。
...
### [93] [Generate-Then-Validate: A Novel Question Generation Approach Using Small Language Models](https://arxiv.org/abs/2512.10110)
*Yumou Wei,John Stamper,Paulo F. Carvalho*

Main category: cs.CL

TL;DR: 本文探索使用小型语言模型(SLMs)进行自动问题生成，提出一个结合文本生成和概率推理能力的管道，采用"生成-验证"策略，通过人类专家和大型语言模型评估证明能生成高质量问题。


<details>
  <summary>Details</summary>
Motivation: 学习分析研究中普遍使用大型语言模型进行问题生成，本文探索小型语言模型作为补充，研究如何利用SLMs的文本生成和概率推理能力来生成高质量问题。

Method: 提出新颖的问题生成管道，采用"生成-验证"策略：首先进行扩展性生成创建大量候选问题，然后通过基于新颖概率推理的选择性验证来精炼问题。

Result: 通过两个评估研究（7名人类专家和大型语言模型）评估生成问题质量，大多数评估者（人类或LLM）认为生成的问题具有清晰答案，并且与预期学习目标基本一致。

Conclusion: 研究结果表明，当通过精心设计的管道利用其优势时，小型语言模型能够有效生成高质量问题，为学习分析中的问题生成提供了可行的替代方案。
...
### [94] [Workflow is All You Need: Escaping the "Statistical Smoothing Trap" via High-Entropy Information Foraging and Adversarial Pacing](https://arxiv.org/abs/2512.10121)
*Zhongjie Jiang*

Main category: cs.CL

TL;DR: DeepNews框架通过模拟金融记者的认知过程，解决长文本生成中的"不可能三角"问题，显著降低幻觉率并提升逻辑连贯性


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在垂直领域长文本生成中存在"不可能三角"困境：难以同时实现低幻觉率、深度逻辑连贯性和个性化表达。研究发现这是由于现有生成范式陷入了"统计平滑陷阱"，忽略了专家级写作所需的高熵信息获取和结构化认知过程。

Method: 提出DeepNews框架，模拟资深金融记者的认知过程，包含三个核心模块：1)基于信息觅食理论的双粒度检索机制，强制10:1的饱和信息输入比；2)基于领域专家知识库(叙事模式)和原子块的模式引导战略规划；3)使用节奏打破和逻辑迷雾等策略的对抗性约束提示技术。

Result: 实验发现深度财经报道中存在明显的"知识悬崖"：当检索上下文低于15000字符时内容真实性崩溃，而超过30000字符的高冗余输入可将无幻觉率稳定在85%以上。在顶级中文科技媒体的生态效度盲测中，基于上一代模型的DeepNews系统获得了25%的投稿接受率，显著优于SOTA模型GPT-5的0%接受率。

Conclusion: 通过显式建模专家的认知过程，DeepNews框架有效解决了长文本生成中的"不可能三角"问题，证明了结构化认知模拟在提升大语言模型专业领域文本生成质量方面的有效性。
...
### [95] [PARAN: Persona-Augmented Review ANswering system on Food Delivery Review Dataset](https://arxiv.org/abs/2512.10148)
*Moonsoo Park,Jeongseok Yun,Bohyung Kim*

Main category: cs.CL

TL;DR: 本文提出了一种两阶段提示框架，通过从短评论文本中推断用户显性和隐性人设特征，来增强个性化回复生成，无需微调模型即可提升回复的相关性和个性化程度。


<details>
  <summary>Details</summary>
Motivation: 在用户信息有限的领域（如外卖平台），个性化回复生成面临挑战。大型语言模型（LLMs）虽然具备强大的文本生成能力，但缺乏上下文用户数据时往往产生通用回复，降低了用户参与度和回复效果。

Method: 提出两阶段提示框架：1）从短评论文本中推断用户显性（如陈述的偏好）和隐性（如人口统计或风格线索）人设特征；2）将这些推断的人设属性融入回复生成提示中，生成用户定制的回复。为鼓励多样且忠实的生成，在推理过程中调整解码温度。

Result: 使用韩国外卖应用的真实数据集进行评估，结果显示该方法在精确度、多样性和语义一致性方面均有提升，证明了人设增强提示在提升自动回复相关性和个性化方面的有效性。

Conclusion: 人设增强提示框架有效提升了在有限用户信息场景下的个性化回复生成质量，无需模型微调即可实现更好的相关性和个性化，为实际应用提供了实用解决方案。
...
### [96] [AutoMedic: An Automated Evaluation Framework for Clinical Conversational Agents with Medical Dataset Grounding](https://arxiv.org/abs/2512.10195)
*Gyutaek Oh,Sangjoon Park,Byung-Hoon Kim*

Main category: cs.CL

TL;DR: AutoMedic是一个用于评估临床对话LLM的多智能体仿真框架，通过将静态QA数据集转化为虚拟患者档案，实现自动化的多轮临床对话评估。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在医疗领域的评估主要依赖静态QA基准，缺乏对动态交互式临床对话场景的评估，且评估维度单一（仅关注准确性），无法全面衡量LLM在实际临床对话中的表现。

Method: 提出AutoMedic多智能体仿真框架：1）将现有静态QA数据集转化为虚拟患者档案；2）创建LLM智能体进行多轮临床对话；3）使用CARE指标从准确性、效率/策略、同理心和鲁棒性四个维度进行评估。

Result: AutoMedic能够有效评估临床对话LLM，经人类专家验证，该框架能够提供可靠的自动化评估，为医疗对话应用的LLM开发提供实用指导。

Conclusion: AutoMedic填补了医疗LLM动态交互评估的空白，通过多智能体仿真和CARE多维度指标，为临床对话LLM的开发提供了标准化、自动化的评估框架。
...
### [97] [Multilingual VLM Training: Adapting an English-Trained VLM to French](https://arxiv.org/abs/2512.10336)
*Jules Lahmi,Alexis Roger*

Main category: cs.CL

TL;DR: 本文探讨了将英语训练的视觉-语言模型适应到其他语言的方法，通过比较翻译管道、LoRA微调和两阶段微调策略，发现数据集翻译是主要瓶颈，数据质量限制了训练和评估效果。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言模型在近年来取得了显著进展，但这些进展主要局限于英语，降低了非英语用户的可访问性。有必要将这些能力扩展到更广泛的语言范围。

Method: 探索并比较了三种适应方法：基于翻译的管道方法、LoRA微调方法，以及将视觉适应与语言适应分离的两阶段微调策略。使用翻译后的多模态基准测试和母语专家手动评估来评估这些方法。

Result: 结果表明，数据集翻译是多语言VLM性能的主要瓶颈，数据质量限制了训练和评估的有效性。

Conclusion: 未来工作应专注于收集本语言数据集和改进翻译策略，以提高多语言视觉-语言模型的性能和可访问性。
...
### [98] [Confucius Code Agent: An Open-sourced AI Software Engineer at Industrial Scale](https://arxiv.org/abs/2512.10398)
...
TL;DR: Confucius Code Agent (CCA)是一个开源AI软件工程师，在工业规模任务上表现优异，在SWE-Bench-Pro上达到54.3%的Resolve@1性能，超越现有编码智能体。


<details>
  <summary>Details</summary>
Motivation: 现实世界AI软件工程需要编码智能体能够处理大规模代码库、保持跨会话持久记忆、并协调复杂工具链。现有开源编码智能体在工业规模任务上表现不足，而专有编码智能体虽然性能强但可扩展性、可解释性和可控性有限。

Method: 基于Confucius SDK开发，该平台包含三个互补视角：智能体体验(AX)、用户体验(UX)和开发者体验(DX)。采用统一编排器与分层工作记忆进行长上下文推理，持久笔记系统实现跨会话持续学习，模块化扩展模块支持稳健工具使用。元智能体通过构建-测试-改进循环自动合成、评估和优化智能体配置。

Result: 在SWE-Bench-Pro基准测试中，CCA实现了54.3%的Resolve@1性能，达到最先进水平，显著超越之前的编码智能体。

Conclusion: Confucius SDK和CCA共同提供了一个透明、可扩展、可复现的AI智能体基础，弥合了研究原型与生产级系统之间的差距，支持工业规模的智能体开发和部署。
...
### [99] [Sliding Window Attention Adaptation](https://arxiv.org/abs/2512.10411)
...
### [100] [Cooperative Retrieval-Augmented Generation for Question Answering: Mutual Information Exchange and Ranking by Contrasting Layers](https://arxiv.org/abs/2512.10422)
...
### [101] [T-pro 2.0: An Efficient Russian Hybrid-Reasoning Model and Playground](https://arxiv.org/abs/2512.10430)
...
### [102] [Semantic Reconstruction of Adversarial Plagiarism: A Context-Aware Framework for Detecting and Restoring "Tortured Phrases" in Scientific Literature](https://arxiv.org/abs/2512.10435)
...
### [103] [Enhancing Next-Generation Language Models with Knowledge Graphs: Extending Claude, Mistral IA, and GPT-4 via KG-BERT](https://arxiv.org/abs/2512.10440)
...
### [104] [Decoding Student Minds: Leveraging Conversational Agents for Psychological and Learning Analysis](https://arxiv.org/abs/2512.10441)
...
### [105] [Grammaticality Judgments in Humans and Language Models: Revisiting Generative Grammar with LLMs](https://arxiv.org/abs/2512.10453)
*Lars G. B. Johnsen*
...
### [106] [XDoGE: Multilingual Data Reweighting to Enhance Language Inclusivity in LLMs](https://arxiv.org/abs/2512.10545)
...
### [107] [Causal Reasoning Favors Encoders: On The Limits of Decoder-Only Models](https://arxiv.org/abs/2512.10561)
...
### [108] [RoleRMBench & RoleRM: Towards Reward Modeling for Profile-Based Role Play in Dialogue Systems](https://arxiv.org/abs/2512.10575)
...
### [109] [AgriGPT-Omni: A Unified Speech-Vision-Text Framework for Multilingual Agricultural Intelligence](https://arxiv.org/abs/2512.10624)
...
### [110] [From Data Scarcity to Data Care: Reimagining Language Technologies for Serbian and other Low-Resource Languages](https://arxiv.org/abs/2512.10630)
...
### [111] [Textual Data Bias Detection and Mitigation - An Extensible Pipeline with Experimental Evaluation](https://arxiv.org/abs/2512.10734)
...
### [112] [Long-horizon Reasoning Agent for Olympiad-Level Mathematical Problem Solving](https://arxiv.org/abs/2512.10739)
...
### [113] [TRIDENT: A Redundant Architecture for Caribbean-Accented Emergency Speech Triage](https://arxiv.org/abs/2512.10741)
...
### [114] [OPV: Outcome-based Process Verifier for Efficient Long Chain-of-Thought Verification](https://arxiv.org/abs/2512.10756)
...
### [115] [Grow Up and Merge: Scaling Strategies for Efficient Language Adaptation](https://arxiv.org/abs/2512.10772)
...
### [116] [Script Gap: Evaluating LLM Triage on Indian Languages in Native vs Roman Scripts in a Real World Setting](https://arxiv.org/abs/2512.10780)
...
### [117] [The FACTS Leaderboard: A Comprehensive Benchmark for Large Language Model Factuality](https://arxiv.org/abs/2512.10791)
...
### [118] [LabelFusion: Learning to Fuse LLMs and Transformer Classifiers for Robust Text Classification](https://arxiv.org/abs/2512.10793)
...
### [119] [Unsupervised Acquisition of Discrete Grammatical Categories](https://arxiv.org/abs/2503.18702)
...
### [121] [Suzume-chan: Your Personal Navigator as an Embodied Information Hub](https://arxiv.org/abs/2512.09932)
...
### [122] [Exploring Health Misinformation Detection with Multi-Agent Debate](https://arxiv.org/abs/2512.09935)
...
### [123] [Echo-CoPilot: A Multi-View, Multi-Task Agent for Echocardiography Interpretation and Reporting](https://arxiv.org/abs/2512.09944)
...
### [124] [Fuzzy Hierarchical Multiplex](https://arxiv.org/abs/2512.09976)
*Alexis Kafantaris*
...
### [125] [Exploring LLMs for Scientific Information Extraction Using The SciEx Framework](https://arxiv.org/abs/2512.10004)
...
### [126] [DynaMate: An Autonomous Agent for Protein-Ligand Molecular Dynamics Simulations](https://arxiv.org/abs/2512.10034)
...
### [127] [SimWorld-Robotics: Synthesizing Photorealistic and Dynamic Urban Environments for Multimodal Robot Navigation and Collaboration](https://arxiv.org/abs/2512.10046)
...
### [128] [Parallel Decoder Transformer: Model-Internal Parallel Decoding with Speculative Invariance via Note Conditioning](https://arxiv.org/abs/2512.10054)
*Logan Robbins*
...
### [131] [Interpretable Embeddings with Sparse Autoencoders: A Data Analysis Toolkit](https://arxiv.org/abs/2512.10092)
...
### [132] [Robust AI Security and Alignment: A Sisyphean Endeavor?](https://arxiv.org/abs/2512.10100)
*Apostol Vassilev*
...
### [133] [Modeling Narrative Archetypes in Conspiratorial Narratives: Insights from Singapore-Based Telegram Groups](https://arxiv.org/abs/2512.10105)
...
### [134] [AgriRegion: Region-Aware Retrieval for High-Fidelity Agricultural Advice](https://arxiv.org/abs/2512.10114)
...
### [135] [The 2025 Foundation Model Transparency Index](https://arxiv.org/abs/2512.10169)
...
### [136] [CP-Env: Evaluating Large Language Models on Clinical Pathways in a Controllable Hospital Environment](https://arxiv.org/abs/2512.10206)
...
### [137] [An exploration for higher efficiency in multi objective optimisation with reinforcement learning](https://arxiv.org/abs/2512.10208)
*Mehmet Emin Aydin*
...
### [138] [ID-PaS : Identity-Aware Predict-and-Search for General Mixed-Integer Linear Programs](https://arxiv.org/abs/2512.10211)
...
### [139] [Reverse Thinking Enhances Missing Information Detection in Large Language Models](https://arxiv.org/abs/2512.10273)
...
### [140] [Neuronal Attention Circuit (NAC) for Representation Learning](https://arxiv.org/abs/2512.10282)
...
### [141] [Investigating The Functional Roles of Attention Heads in Vision Language Models: Evidence for Reasoning Modules](https://arxiv.org/abs/2512.10300)
...
### [142] [Planning, Living and Judging: A Multi-agent LLM-based Framework for Cyclical Urban Planning](https://arxiv.org/abs/2412.20505)
...
### [143] [Trustworthy Orchestration Artificial Intelligence by the Ten Criteria with Control-Plane Governance](https://arxiv.org/abs/2512.10304)
...
### [144] [InfoCom: Kilobyte-Scale Communication-Efficient Collaborative Perception with Information Bottleneck](https://arxiv.org/abs/2512.10305)
...
### [145] [EpiPlanAgent: Agentic Automated Epidemic Response Planning](https://arxiv.org/abs/2512.10313)
...
### [146] [User-Feedback-Driven Continual Adaptation for Vision-and-Language Navigation](https://arxiv.org/abs/2512.10322)
...
### [147] [On the Collapse of Generative Paths: A Criterion and Correction for Diffusion Steering](https://arxiv.org/abs/2512.10339)
...
### [148] [When Reject Turns into Accept: Quantifying the Vulnerability of LLM-Based Scientific Reviewers to Indirect Prompt Injection](https://arxiv.org/abs/2512.10449)
...
### [151] [LLM-Empowered Representation Learning for Emerging Item Recommendation](https://arxiv.org/abs/2512.10370)
...
### [153] [AgentProg: Empowering Long-Horizon GUI Agents with Program-Guided Context Management](https://arxiv.org/abs/2512.10371)
...
### [154] [Boosting RL-Based Visual Reasoning with Selective Adversarial Entropy Intervention](https://arxiv.org/abs/2512.10414)
...
### [155] [Representation of the structure of graphs by sequences of instructions](https://arxiv.org/abs/2512.10429)
...
### [156] [Targeted Data Protection for Diffusion Model by Matching Training Trajectory](https://arxiv.org/abs/2512.10433)
...
### [163] [On the Dynamics of Multi-Agent LLM Communities Driven by Value Diversity](https://arxiv.org/abs/2512.10665)
...
### [164] [AEBNAS: Strengthening Exit Branches in Early-Exit Networks through Hardware-Aware Neural Architecture Search](https://arxiv.org/abs/2512.10671)
...
### [165] [Challenges of Evaluating LLM Safety for User Welfare](https://arxiv.org/abs/2512.10687)
...
### [166] [Enhancing Radiology Report Generation and Visual Grounding using Reinforcement Learning](https://arxiv.org/abs/2512.10691)
...
### [167] [COMPARE: Clinical Optimization with Modular Planning and Assessment via RAG-Enhanced AI-OCT: Superior Decision Support for Percutaneous Coronary Intervention Compared to ChatGPT-5 and Junior Operators](https://arxiv.org/abs/2512.10702)
...
### [168] [HAROOD: A Benchmark for Out-of-distribution Generalization in Sensor-based Human Activity Recognition](https://arxiv.org/abs/2512.10807)
...
### [169] [Agile Deliberation: Concept Deliberation for Subjective Visual Classification](https://arxiv.org/abs/2512.10821)
...
### [170] [V-OCBF: Learning Safety Filters from Offline Data via Value-Guided Offline Control Barrier Functions](https://arxiv.org/abs/2512.10822)
...
### [171] [DeMapGS: Simultaneous Mesh Deformation and Surface Attribute Mapping via Gaussian Splatting](https://arxiv.org/abs/2512.10572)
*Shuyi Zhou,Shengze Zhong,Kenshi Takayama,Takafumi Taketomi,Takeshi Oishi*

Main category: cs.GR

TL;DR: DeMapGS是一个结构化高斯泼溅框架，通过将2D高斯泼溅锚定到可变形模板网格上，联合优化可变形表面和表面附着的高斯泼溅，解决拓扑不一致问题并增强编辑灵活性。


<details>
  <summary>Details</summary>
Motivation: 传统高斯泼溅方法将点视为独立处理，存在拓扑不一致和编辑灵活性不足的问题。作者希望创建一种统一表示，既能保持高斯泼溅的逼真渲染质量，又能支持高质量的网格重建和编辑应用。

Method: 1. 将2D高斯泼溅锚定到可变形模板网格表面
2. 引入梯度扩散策略，在表面上传播监督信号
3. 采用交替的2D/3D渲染方案处理凹面区域
4. 联合优化可变形表面和表面附着的高斯泼溅

Result: DeMapGS实现了最先进的网格重建质量，能够提取高保真的漫反射、法线和位移贴图。重建的网格继承了高斯泼溅的逼真渲染质量，并支持高斯泼溅的下游应用，如编辑和跨对象操作。

Conclusion: DeMapGS通过结构化表示解决了高斯泼溅方法的拓扑不一致问题，提供了统一的表面表示，支持高质量的网格重建和灵活的编辑操作，为高斯泼溅技术开辟了新的应用可能性。
